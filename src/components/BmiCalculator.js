...
const BmiCalc = (height, weight) => {
  ...
  if (result < 18.5) {
    document.querySelector(
      ".range-bmi"
    ).textContent = `You Are in UnderWeight Range`;
    document.querySelector(".range-bmi").classList.add("red");
  } else if (result > 18.5 && result < 24.9) {
    document.querySelector(
      ".range-bmi"
    ).textContent = `You Fall in Healthy Weight Range`;
    document.querySelector(".range-bmi").classList.add("green");
  } else {
    document.querySelector(
      ".range-bmi"
    ).textContent = `You Fall in Obese Range`;
    document.querySelector(".range-bmi").classList.add("red");
  }
};
...
const BmiCalculator = () => {
  ...
  return (
    <div className="py-8 w-full bg-slate-200 font-poppins">
      <div className="w-11/12 mx-auto flex flex-col items-center">
        <h1 className="text-4xl">Get Started with our BMI Calculator.</h1>
        <div
          className="flex flex-col gap-8 border border-slate-400 
        ...
        >
          ...
          <input
            ...
            className="border border-blue-500 px-4 py-2 rounded-md active:border-blue-400"
          ></input>
          ...
          {/*  SUBMIT BUTTON */}
          <button
            onClick={() => BmiCalc(height, weight)}
            className="bg-blue-500 py-3 px-4 w-6/12 mx-auto text-white font-bold rounded-md"
          >
            Submit
          </button>
